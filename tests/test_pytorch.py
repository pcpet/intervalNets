...
def test_is_affine_on_detects_stable_relu_pattern() -> None:
    enable_interval_eval()
    model = nn.Sequential(nn.Linear(1, 2), nn.ReLU(), nn.Linear(2, 1))
    with torch.no_grad():
        model[0].weight.copy_(torch.tensor([[1.0], [2.0]]))
        model[0].bias.copy_(torch.tensor([1.0, 0.5]))
        model[2].weight.copy_(torch.tensor([[1.0, -1.0]]))
        model[2].bias.copy_(torch.tensor([0.0]))

    domain = IntervalTensor.from_bounds([0.0], [1.0])
    assert model.is_affine_on(domain) is True


def test_is_affine_on_detects_crossing_relu_pattern() -> None:
    enable_interval_eval()
    model = nn.Sequential(nn.Linear(1, 1), nn.ReLU(), nn.Linear(1, 1))
    with torch.no_grad():
        model[0].weight.copy_(torch.tensor([[1.0]]))
        model[0].bias.copy_(torch.tensor([0.0]))
        model[2].weight.copy_(torch.tensor([[2.0]]))
        model[2].bias.copy_(torch.tensor([0.0]))
