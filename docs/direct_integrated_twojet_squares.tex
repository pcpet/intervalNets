...
The proposed method is algebraically exact: with the same approximation-noise
...
\[
    ...
    \alpha\in[-1,1]^d,
\]
and its nonnegative geometric Jacobian density
\[
    J_X:=\abs{\det\left.\partial X\right|_\alpha}
        =\abs{\det G}
\]
is constant.  For an axis-aligned box, \(J_X\) is the product of the
...
\[
    \eps=(\alpha,\eta)
\]
collect the two classes of noise symbols used throughout these notes:
\begin{enumerate}[label=\arabic*.]
    \item \(\alpha\): domain noise symbols, integrated over \([-1,1]^d\);
    \item \(\eta\): approximation noise symbols introduced by certified
...
A generic monomial is written as
\[
    \eps^\lambda
    =\alpha^{\lambda^\alpha}\eta^{\lambda^\eta},
    \qquad
    \lambda=(\lambda^\alpha,\lambda^\eta).
\]
Letters such as \(\beta,\delta,\gamma\) below denote additional
multi-indices when several exponent keys occur in the same formula.

There is no additional class of residual or pointwise-error symbols: the
approximation noise symbols \(\eta\) carry precisely the certified activation
approximation uncertainty.  Under the integration semantics used here, their
values may represent the residual function at each physical point.
Consequently, a monomial containing approximation noise is not integrated by
the ordinary signed monomial-moment formula.
...
\[
    ...
    \qquad
    P:=\{\text{pointwise approximation-noise indices}\},
\]
and let \(U:=\{1,\ldots,p\}\setminus D\) collect all non-domain uncertainty
indices.  Retained global symbolic indices may belong to \(U\setminus P\); in
the common two-class convention, \(U=P\).  For an exponent \(\gamma\), denote
by \(\gamma_D\) and \(\gamma_U\) the corresponding restrictions.

For terms containing no approximation noise, define the box moment
\begin{equation}
    \mu_{\gamma_D}
    :=
...
\[
    M:=2^d.
\]
For pointwise terms, the relevant domain functional is instead the absolute
box moment
\begin{equation}
    \mu^{\mathrm{abs}}_{\gamma_D}
    :=
    \int_{[-1,1]^d}\abs{\alpha^{\gamma_D}}\,d\alpha
    =
    \prod_{i\in D}\frac{2}{\gamma_i+1}.
    \label{eq:absolute-moment}
\end{equation}
There is no parity cancellation in \eqref{eq:absolute-moment}.

Canonical coefficients with the same full exponent \(\gamma\) are combined
before the following rule is applied.  If a term contains pointwise
approximation noise, let \(m_\gamma\) and \(r_\gamma\) denote its integrated
midpoint and symmetric radius.  With \(U=\{1,\ldots,p\}\setminus D\), the
parity-aware coefficientwise rule is
\begin{equation}
 (m_\gamma,r_\gamma)
 =J_X\begin{cases}
 \left(\frac12q_\gamma\mu^{\mathrm{abs}}_{\gamma_D},
       \frac12\abs{q_\gamma}\mu^{\mathrm{abs}}_{\gamma_D}\right),
 &\gamma_i\text{ even for every }i\in U
  \text{ and every }i\in D,\\[2mm]
 \left(0,
       \frac12\abs{q_\gamma}\mu^{\mathrm{abs}}_{\gamma_D}\right),
 &\gamma_i\text{ even for every }i\in U
  \text{ and some }\gamma_i, i\in D,\text{ is odd},\\[2mm]
 \left(0,
       \abs{q_\gamma}\mu^{\mathrm{abs}}_{\gamma_D}\right),
 &\text{otherwise}.
 \end{cases}
 \label{eq:pointwise-parity-rule}
\end{equation}
The first case uses that the entire monomial lies in \([0,1]\).  In the
second case the uncertainty factor lies in \([0,1]\), while the positive and
negative parts of the odd domain monomial have equal integral.  The last case
retains the symmetric \([-1,1]\) bound because at least one non-domain
uncertainty power is odd.  If a retained global symbolic factor cannot be
classified safely, an implementation must use the last case.

The improved \texttt{pointwise\_interval} semantics therefore produce
\begin{align}
    b_\nu
    &:=
    J_X
    \sum_{\substack{\gamma:\,\gamma_U=\nu\\
                    \gamma_i=0\ \forall i\in P}}
    q_\gamma\mu_{\gamma_D},
    \label{eq:retained-coefficients}\\
    M_{\mathrm{app}}
    &:=\sum_{\substack{\gamma:\\
                    \exists i\in P:\ \gamma_i>0}}m_\gamma,
    &
    R_{\mathrm{app}}
    &:=\sum_{\substack{\gamma:\\
                    \exists i\in P:\ \gamma_i>0}}r_\gamma.
    \label{eq:approximation-radius}
\end{align}
Here \(q_0\) may be viewed as the coefficient with zero exponent, so its
contribution is included in \(b_0\).  Under the two-class convention,
\(\gamma_i=0\) for every \(i\in P\) implies \(\gamma_U=0\); hence the
retained object is the scalar \(B=b_0\).  The more general coefficient-map
notation is kept below because it matches the implementation.
...
\begin{warning}
For an approximation-noise term, the rule uses the absolute moment
\(\mu^{\mathrm{abs}}_{\gamma_D}\), not the possibly vanishing signed moment
\(\mu_{\gamma_D}\).  Thus an odd domain factor multiplying approximation
noise is retained.  The rule remains coefficientwise and is generally not
the sharp pointwise integral enclosure.
\end{warning}
...
\begin{enumerate}[label=\arabic*.]
    \item \texttt{approximation\_coeff[\(\gamma\)]}: the canonical coefficient of
    every full exponent containing approximation noise;
    \item \texttt{retained\_coeff[\(\nu\)]}: the already integrated
    coefficient of every retained exponent \(\nu=\gamma_R\) for terms
    containing no approximation noise.
\end{enumerate}
...
=
\begin{cases}
    \texttt{approximation\_coeff[\(\gamma\)]}
        \mathrel{+}= J_X s,
        &\exists i\in P:\gamma_i>0,\\[1mm]
    \text{discard},
        &\mu_{\gamma_D}=0,\\[1mm]
    \texttt{retained\_coeff[\(\gamma_R\)]}
        \mathrel{+}= J_X\mu_{\gamma_D}s,
        &\text{otherwise}.
\end{cases}
\label{eq:routing}
\end{equation}
...
\begin{align}
    ...
    &:=
    M\sum_{\gamma}
        \abs{\texttt{approximation\_coeff[\(\gamma\)]}},
    \label{eq:direct-approximation-radius}\\
    b_0
    &:=
    ...
    &:=
    \sum_{\nu\neq0}
        \abs{\texttt{retained\_coeff[\(\nu\)]}}.
    \label{eq:direct-symbolic-radius}
\end{align}
The final interval is
\begin{equation}
    \boxed{
    [b_0-R_{\mathrm{sym}}-R_{\mathrm{app}},
     b_0+R_{\mathrm{sym}}+R_{\mathrm{app}}].
    }
...
\(b_\nu\) as collecting the full squared polynomial first and integrating
afterward.  For approximation-noise terms, the direct algorithm retains the full exponent
key and collects every contribution before taking its absolute value.
Consequently \eqref{eq:direct-approximation-radius} equals
\eqref{eq:approximation-radius}.  The final interval-enclosure step is identical to
\eqref{eq:current-functional}.
\end{proof}
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Using the domain moment of an approximation-noise term is inconsistent
    with the present approximation-noise semantics and can be unsound.
...
\(J_X\geq0\), domain indices \(D\), and approximation-noise indices \(P\).
Proceed as follows.
\begin{enumerate}[label=\arabic*.,leftmargin=1.8em]
    ...
    \item Initialize
    \[
        \texttt{approximation\_coeff}\gets\{\},\qquad
        \texttt{retained\_coeff}
        \gets\{0:J_X 2^{\abs D}c^\top\diag(w)c\}.
    \]
...
\begin{enumerate}[label=\arabic*.]
    ...
    \texttt{retained\_coeff};
    \item store \(R_{\mathrm{app}}\) as its \texttt{interval\_radius};
    \item call the existing \texttt{interval\_enclosure()} method.
\end{enumerate}
Under the two-class convention this post-integration object has only its
constant coefficient; it is not the 55,992-term squared integrand over all
noise symbols.  Reusing the existing method also preserves the established
outward-padding behavior.
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item obtain \(P\), the approximation-noise index set, using the same predicate and the same
    \texttt{POINTWISE\_RESIDUAL\_KINDS} constant as the current integrator;
    \item retain every index outside \(D\), including approximation-noise indices in the
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item discarding odd domain-only moments immediately;
    \item collapsing domain-only terms directly to retained exponents;
...
Use the approximation noise symbol \(\eta\) and two coordinates
\[
    z_1=1+\eta,
    \qquad
    z_2=1-\eta.
...
\]
The two linear \(\eta\)-coefficients cancel before the approximation-noise radius is
formed.  A faulty pairwise-absolute-value implementation would lose this
cancellation.

\subsection{Approximation noise multiplied by an odd domain monomial}
...
is zero, but the pointwise semantics assign it a nonzero interval radius using
\(\int_{-1}^1\abs{\alpha}\,d\alpha=1\), rather than the total measure two.
The \(\eta^2\) term is nonnegative and therefore receives the one-sided
midpoint/radius reduction.  The direct implementation must match the explicit
squared-integrand result after both apply these rules.
...
\begin{aligned}
    ...
    \\
    &\longrightarrow
    \text{canonicalize directly into approximation-noise and integrated accumulators}
    \\
    &\longrightarrow
...
coefficients of equal approximation-noise monomials must still be accumulated
...
For deep graphs where the exact canonical support is itself prohibitive, the
separate note
\texttt{dependency\_preserving\_graph\_hilbert\_integration.tex} specifies
the implemented certified-compression backend.  That method keeps an immutable
shared arithmetic graph, validates small cases against exact sparse moments,
and replaces large intermediate fields by an orthogonal affine projection plus
an explicit $L^2$ remainder.  It is not claimed to reproduce this direct
coefficientwise functional exactly.
