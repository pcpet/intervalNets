...
\[
    ...
    \qquad
    P:=\{\text{approximation-noise indices}\},
\]
and let \(R:=\{1,\ldots,p\}\setminus D=P\).  Thus there are no
non-domain symbolic variables beyond the approximation noise symbols.
...
\(\gamma_i=0\) for every \(i\in P\) implies \(\gamma_R=0\); hence the
...
is zero, but the current approximation-noise semantics assign it a nonzero interval
